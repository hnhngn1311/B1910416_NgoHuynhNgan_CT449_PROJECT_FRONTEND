        ...
        <div class="d-flex flex-column">
          <div class="d-flex">
            <div class="d-flex gap-4 bg-light p-2">
              ...
              <div class="mb-3">
                ...
                <select class="form-select form-select-lg" name="" v-model="selectedTypeRoom" id="">
                  <option selected :value="null">Tất cả</option>
                  <option v-for="item in typesRoom" :value="item.type" :key="item.id">
                    {{ item.type }}
                  </option>
                </select>
              </div>
          ...
          <div class="d-flex flex-column gap-1">
            ...
            <div class="d-flex flex-wrap gap-1">
              <button
                ...
                :class="{ 'btn-success text-light': bookingData.room === i.name }"
          ...
          <div class="d-flex flex-column gap-1">
            ...
            <div class="d-flex gap-1">
              <button
                ...
                @click="bookingData.startDate = moment(item.date).set('hours',0).set('minute', 0)"
              >
          ...
          <div class="d-flex flex-column gap-1">
            ...
            <div class="d-flex gap-1">
              <button
                ...
                :disabled="moment(item.start, 'HH:mm') >= moment(endLesson, 'HH:mm')"
              >
            ...
            <div class="d-flex gap-1">
              <button
                ...
                :disabled="moment(item.end, 'HH:mm') <= moment(startLesson, 'HH:mm')"
                :key="item._id"
          ...
          <div class="d-flex justify-content-between mx-4 mt-4">
            <div class="mb-3">
              ...
              <textarea class="form-control" name="" id="" cols="100" rows="4"></textarea>
            </div>
          </div>
          <div class="d-flex justify-content-between mx-4 mt-4">
            <div class="d-flex fw-bold">
              Đặt phòng {{ bookingData.room }} vào lúc
              "{{ moment(bookingData.startDate).format('yyyy-MM-DD HH:mm') }}" đến "{{ moment(bookingData.endDate).format('yyyy-MM-DD HH:mm') }}"
            </div>
            <button type="button" class="btn btn-outline-success" @click="handleBooking()">Đặt phòng</button>
          </div>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    setStartDate(date){
      ...
      var newDate = moment(new Date(this.bookingData.startDate)).set('hour',moment(this.startLesson, 'HH:mm').get('hour')).set('minute',moment(this.startLesson, 'HH:mm').get('minute')).format('yyyy-MM-DD HH:mm')
      this.bookingData.startDate = newDate;
    },
    setEndDate(date){
      this.endLesson = date;
      var newDate = moment(new Date(this.bookingData.startDate)).set('hour',moment(this.endLesson, 'HH:mm').get('hour')).set('minute',moment(this.endLesson, 'HH:mm').get('minute') )
